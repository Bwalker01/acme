...
public class ItemResponse {
    ...
    public ItemResponse(HashMap<Product, Integer> inputItems, HashMap<DiscountBundle, Integer> inputDiscount) {
        ...
        for (Product product : inputItems.keySet()) {
            items.add(new OutputProduct(product.getName(), product.getPrice() * inputItems.get(product),
                    inputItems.get(product)));
        }

        // for (DiscountBundle discountBundle : inputDiscount.keySet()) {
        // items.add(new OutputProduct(discountBundle.getName(),
        // discountBundle.getPrice() * inputItems.get(discountBundle),
        // inputItems.get(discountBundle)));
        // }
